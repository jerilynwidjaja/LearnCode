...
export class ProgressService {
  ...
  static async getAnalytics(userId) {
    ...
    filteredProgress.forEach(p => {
      if (p.question?.course?.category) {
        const category = p.question.course.category;
    ...
    const recentActivity = filteredProgress
      .filter(p => p.lastAttemptAt && p.question?.title && p.question?.course?.title &&
        new Date(p.lastAttemptAt) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000))
      .map(p => ({
        questionTitle: p.question.title,
        courseTitle: p.question.course.title,
